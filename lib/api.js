...
async function handle(ctx) {
  ...
  if (pathname === '/notify' && method === 'POST') {
    ...
    const ip = (ctx.req.headers['x-forwarded-for'] || ctx.req.socket.remoteAddress || 'unknown').toString().split(',')[0].trim();
    const now = Date.now();
    const BUCKET = (notifyBuckets.get(ip) || []).filter(t => now - t < 60_000);
    if (BUCKET.length >= 6) return fail(res, 'Too many requests — try again in a minute.', 429);
    BUCKET.push(now); notifyBuckets.set(ip, BUCKET);
