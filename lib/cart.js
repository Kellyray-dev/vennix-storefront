...
function clearCheckoutReuse(cart) {
  if (cart._lastOrderId || cart._converting || cart.status === 'converted') {
    delete cart._lastOrderId;
    delete cart._converting;
    delete cart._checkoutToken;
    if (cart.status === 'converted') cart.status = 'active';
    delete cart.orderId;
  }
}
...
function addItem(cart, variantId, quantity = 1, options = {}) {
  ...
  // All validation passed — now it is safe to drop the completed-checkout
  // marker. Invalid mutations (sold-out, bad monogram, stock cap) leave
  // _lastOrderId intact so a retry of /checkout still redirects to the order.
  clearCheckoutReuse(cart);
...
function addItems(cart, items = []) {
  ...
  // addItem already clears checkout reuse only on success, so a batch where
  // every line fails leaves _lastOrderId intact for idempotent checkout retry.
