...
function money(cents, currency) {
  return moneyLib.money(cents, currency || settings.get().currency || 'USD');
}
...
function brand(settingsArg) {
  const s = settingsArg || settings.get() || {};
  return s.brandName || 'Vennix';
}
...
function wordMark(settingsArg) {
  const s = settingsArg || settings.get() || {};
  const parts = String(s.brandName || 'Vennix').trim().split(/\s+/);
...
function freeShipText(settingsArg) {
  const s = settingsArg || settings.get() || {};
  return money(s.freeShippingThreshold || 0);
}
...
function badgesFor(product) {
  ...
  if (stock > 0 && stock <= (settings.get().lowStockThreshold || 5)) out.push({ label: `Only ${stock} left`, cls: 'badge--low' });
...
function productCard(product, { quickAdd = true, index = 0 } = {}) {
  ...
  <div class="pcard__media">
    <a href="/products/${attr(product.handle)}" class="pcard__link" aria-label="${attr(product.title)}">
      <img class="pcard__img pcard__img--main" src="${attr(product.images[0].src)}" alt="${attr(product.images[0].alt || product.title)}" loading="lazy" decoding="async" width="800" height="800">
      <img class="pcard__img pcard__img--hover" src="${attr(hover)}" alt="" loading="lazy" decoding="async" width="800" height="800" aria-hidden="true">
    </a>
