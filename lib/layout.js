...
/**
 * Word mark. Splits the configured brand into two registers ("Vennix" +
 * "Athletic") so the monogram keeps its two-tone treatment after a rebrand.
 */
...
function navModel() {
  const collections = store.all('collections');
...
function socialLinks(settings) {
  const socials = (settings && settings.socials) || {};
...
function header({ settings, cart, customer, navActive = '', solid = false }) {
  const nav = navModel();
  const wishCount = 0;
  ...
  <header class="site-head ${solid ? 'is-solid' : ''}" data-header>
    <div class="wrap site-head__inner">
      ...
      <a class="brand" href="/" aria-label="${attr(settings.brandName)} home">
      ...
      <div class="head-actions">
        ...
        <a class="icon-btn icon-btn--account" href="${customer ? '/account' : '/account/login'}" aria-label="${customer ? 'Your account' : 'Log in'}">
          ${icon('user')}<span class="icon-btn__label">${customer ? esc(customer.firstName || customer.name || 'Account') : 'Log in'}</span>
        </a>
  ...
  ${mobileMenu(nav, customer)}
  ${searchOverlay()}
  ${cartDrawer(cart, settings)}
  ${quickViewModal()}
  ${wishlistPanel()}
...
function mobileMenu(nav, customer) {
  return `<aside class="mob-menu" id="mobile-menu" data-menu hidden aria-label="Mobile navigation">
    ...
    <div class="mob-menu__foot">
      <a class="btn btn--outline btn--block" href="${customer ? '/account' : '/account/login'}">${customer ? 'Your account' : 'Log in / Register'}</a>
...
function cartDrawer(cart, settings) {
  ...
  return `<aside class="drawer" data-cart-drawer hidden aria-label="Shopping cart" role="dialog" aria-modal="true">
    ...
    <footer class="drawer__foot" data-cart-foot ${cart.count ? '' : 'hidden'}>
      ...
      <p class="drawer__note">Taxes and shipping calculated at checkout. ${settings.payments.testMode ? '<br><em>Sandbox mode — use card 4242 4242 4242 4242.</em>' : ''}</p>
...
function upsellRail(cart) {
  const inCart = new Set(cart.lines.map(l => l.productId));
  const picks = store.all('products')
    .filter(p => !inCart.has(p.id) && p.inventoryQuantity > 0)
  ...
  return `<section class="upsell">
    ...
    <ul class="upsell__list">${picks.map(p => {
      ...
      return `<li>
        <img src="${attr(p.images[0].src)}" alt="" width="80" height="80" loading="lazy">
...
function footer(settings) {
  ...
  const collections = store.all('collections').filter(c => ['women', 'men', 'active', 'essentials', 'new-in'].includes(c.handle));
  return `<footer class="site-foot">
    <div class="wrap site-foot__top">
      ...
      <nav class="site-foot__cols" aria-label="Footer">
        <div><h3>Shop</h3><ul>${collections.map(c => `<li><a href="/collections/${attr(c.handle)}">${esc(c.title)}</a></li>`).join('')}<li><a href="/collections/all">All products</a></li><li><a href="/gift-cards">Gift cards</a></li></ul></div>
        <div><h3>Help</h3><ul>
    ...
    <div class="wrap site-foot__badges">
      ${ui.trustRow(settings.trustBadges, 'trust--foot')}
    </div>
    <div class="wrap site-foot__base">
      <p>© ${year} ${esc(settings.legalName)} · ${esc(settings.address.line1)}, ${esc(settings.address.city)} ${esc(settings.address.province)}</p>
      <ul>
      ...
      <p class="site-foot__pay">Payments: Visa · Mastercard · Amex · Discover · Shop Pay · Apple Pay · Google Pay ${settings.payments.testMode ? ' · <em>sandbox</em>' : ''}</p>
    </div>
  </footer>`;
}

function shell({ title, description, canonical, jsonLd = [], bodyClass = '', content, settings, cart, customer, navActive = '', noHeader = false }) {
  const fullTitle = title ? `${title} | ${settings.brandName}` : settings.seo.title;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<meta name="description" content="${attr(description || settings.seo.description)}">
${canonical ? `<link rel="canonical" href="https://${attr(settings.domain)}${attr(canonical)}">` : ''}
<meta property="og:site_name" content="${attr(settings.brandName)}">
<meta property="og:title" content="${attr(fullTitle)}">
<meta property="og:description" content="${attr(description || settings.seo.description)}">
<meta property="og:type" content="website">
<meta property="og:image" content="https://${attr(settings.domain)}/images/hero-editorial.jpg">
...
<div class="page">
  ${noHeader ? '' : header({ settings, cart, customer, navActive })}
  <main id="main" tabindex="-1">${content}</main>
  ${noHeader ? '' : footer(settings)}
</div>
<script type="application/json" data-catalog-index>${catalogIndexJson()}</script>
...
function catalogIndexJson() {
  const index = store.all('products').filter(p => p.status === 'active').map(p => ({
    ...
    variant: (p.variants.find(v => v.stock > 0) || p.variants[0]).id
  }));
  return JSON.stringify(index).replace(/</g, '\\u003c');
}

module.exports = { shell, header, footer, navModel, cartDrawer, cartBody, cartLine, upsellRail };
