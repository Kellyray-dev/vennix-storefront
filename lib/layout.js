      ...
      <nav class="main-nav" aria-label="Primary">
        <ul>
          ${nav.map(item => `<li class="${item.mega ? 'has-mega' : ''} ${navActive === item.label.toLowerCase() ? 'is-active' : ''}">
            <a href="${attr(item.href)}" ${navActive === item.label.toLowerCase() ? 'aria-current="page"' : ''} ${item.mega ? 'aria-haspopup="true" aria-expanded="false" data-mega-trigger' : ''}>${esc(item.label)}${item.mega ? icon('chevronDown', { size: 14 }) : ''}</a>
 ...
 */
async function shell({ title, description, canonical, jsonLd = [], bodyClass = '', content, settings: sArg, cart, navActive = '', noHeader = false, chrome = null, upsellProducts = [], demo = false, robots = '' }) {
...
${demo ? '' : '<link rel="preconnect" href="https://cdn.shopify.com" crossorigin>'}
${jsonLd.map(j => `<script type="application/ld+json"${nonceAttr()}>${JSON.stringify(j)}</script>`).join('\n')}
</head>
...
<script type="application/json" data-catalog-index${nonceAttr()}>${catalogIndexJson(data.products)}</script>
