 ...
 * rather than only taking effect on the first empty-DB seed. If a password
 * is generated for a production deploy (env=prod, no ADMIN_PASSWORD set),
 * print it once per boot so the operator can sign in.
 */
function applyEnvAdmin() {
  ...
  if (envPassword) {
    ...
    // Production without ADMIN_PASSWORD: rotate to a fresh random password on
    // every boot and print it. This means the old password from a previous
    // boot stops working — if an operator wanted a stable credential they
    // should set ADMIN_PASSWORD; without it they get a rotating secret so a
    // leaked container image or log dump cannot grant persistent access.
    const generated = crypto.randomBytes(6).toString('base64url');
    const pw = auth.hashPassword(generated);
    db.settings.admin.passwordHash = pw.hash;
    db.settings.admin.salt = pw.salt;
    console.log('');
    console.log('  ⚠️  ADMIN BOOT PASSWORD (rotated this boot — ADMIN_PASSWORD not set)');
    console.log(`      Email:    ${db.settings.admin.email}`);
