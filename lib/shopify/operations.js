...
// 2026-07 added viewKeys as an alternative to lineIds — and Shopify validates
// that a call passes EXACTLY ONE of the two arguments (sending both, even with
// one side empty, is rejected with "must include exactly one of the following
// arguments: lineIds, viewKeys"). So each variant gets its own document and
// cart-api picks the matching one per call.
const CART_LINES_REMOVE_BY_IDS = `mutation CartLinesRemove($cartId: ID!, $lineIds: [ID!]) {
  cartLinesRemove(cartId: $cartId, lineIds: $lineIds) { cart { ${CART_FIELDS} } userErrors { field message } }
}`;

const CART_LINES_REMOVE_BY_VIEW_KEYS = `mutation CartLinesRemoveByViewKeys($cartId: ID!, $viewKeys: [String!]) {
  cartLinesRemove(cartId: $cartId, viewKeys: $viewKeys) { cart { ${CART_FIELDS} } userErrors { field message } }
}`;

// 2026-07 signatures: discountCodes is [String!]! and note is String! — a
// nullable variable passed to a non-null argument is rejected with
...
module.exports = {
  ...
  GET_CART,
  CART_CREATE, CART_LINES_ADD, CART_LINES_UPDATE,
  CART_LINES_REMOVE_BY_IDS, CART_LINES_REMOVE_BY_VIEW_KEYS,
