...
const PRODUCT_FIELDS = `
  ...
  media(first: 20, types: [IMAGE]) { nodes { ... on MediaImage { image { ${IMAGE_FIELDS} } } } }
 ...
 * to true, which would hide order-level discounts). Only fields that live on
 * the `BaseCartDiscountApplication` / `CartDiscountAllocation` interfaces are
 * selected, so no inline fragments (and no type-name drift) are involved.
 *
