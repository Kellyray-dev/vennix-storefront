...
/**
 * Remove lines. Accepts line ids, view keys, or a mix of both.
 *
 * Shopify's 2026-07 cartLinesRemove requires EXACTLY ONE of the lineIds /
 * viewKeys arguments per call — sending both (even with one side empty) is
 * rejected before anything runs — so each reference kind is sent in its own
 * mutation using the matching pinned document.
 */
async function removeLines(cartId, refs) {
  ...
  for (const ref of list) {
    ...
    if (ref.id) lineIds.push(ref.id);
    else if (ref.viewKey) viewKeys.push(ref.viewKey);
  }
  if (!lineIds.length && !viewKeys.length) {
    return { ok: false, error: 'removeLines needs at least one line id or viewKey.' };
  }
  let cart = null;
  if (lineIds.length) {
    const data = await gql(ops.CART_LINES_REMOVE_BY_IDS, { cartId, lineIds });
    const result = mutationResult(data, 'cartLinesRemove');
    if (!result.ok) return result;
    cart = result.cart;
  }
  if (viewKeys.length) {
    const data = await gql(ops.CART_LINES_REMOVE_BY_VIEW_KEYS, { cartId, viewKeys });
    const result = mutationResult(data, 'cartLinesRemove');
    if (!result.ok) return result;
    cart = result.cart;
  }
  return { ok: true, cart: N.normalizeCart(cart) };
}
...
async function setDiscountCode(cartId, code) {
  // Always a list, never null: the argument is [String!]! in 2026-07.
...
async function setNote(cartId, note) {
  // note is a non-null argument in 2026-07: clearing it means sending '',
  // never null/undefined.
  const data = await gql(ops.CART_NOTE_UPDATE, { cartId, note: String(note == null ? '' : note) });
