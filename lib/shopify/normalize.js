...
function normalizeArticle(raw) {
  if (!raw) return null;
  // 2026-07: Article.contentHtml (HTML) / Article.content (stripped). The
  // older `body` is accepted only so a cached response still renders.
  const html = raw.contentHtml || raw.content || raw.body || '';
  const excerpt = raw.excerpt || stripHtml(html).slice(0, 180);
  ...
  return {
    ...
    image: raw.image ? raw.image.url : '',
    imageAlt: raw.image ? raw.image.altText || raw.title : '',
    // authorV2 replaced the deprecated author field in 2026-07.
    author: (raw.authorV2 && raw.authorV2.name) || (raw.author && raw.author.name) || settings.get().brandName,
    tags: raw.tags || [],
...
/** Sum the discountedAmount of cart-line discount allocations, ignoring shipping. */
function sumAllocations(allocations) {
  let total = 0;
  for (const a of allocations || []) {
    if (a && a.targetType !== 'SHIPPING_LINE') total += fromMoneyV2(a.discountedAmount) || 0;
  }
  return total;
}
...
function normalizeCart(raw) {
  ...
  const lines = (raw.lines && raw.lines.nodes ? raw.lines.nodes : []).map(line => {
    ...
    return {
      id: line.id,
      // 2026-07: a stable, Liquid-compatible identifier for the line. Sent
      // back on cartLinesUpdate / cartLinesRemove when the line id is unknown.
      viewKey: line.viewKey || null,
      ...
      // Discounts that landed on this line (including order-level ones, which
      // lineLevelOnly: false is required to see). Shipping discounts are
      // deliberately excluded — they do not reduce the merchandise subtotal.
      discountAmount: sumAllocations(line.discountAllocations)
    };
  });
  ...
  // Deprecated in 2026-07 and may disappear in a later version: when it is
  // absent the cart simply stops showing the estimate instead of showing £0.00.
  const tax = fromMoneyV2(cost.totalTaxAmount);
  const discountCodes = (raw.discountCodes || []).filter(d => d.applicable);
  // A plain list in 2026-07 (`{ nodes: … }` is tolerated for cached responses
  // written before the query stopped asking for a connection).
  const rawApplications = Array.isArray(raw.discountApplications)
    ? raw.discountApplications
    : (raw.discountApplications && raw.discountApplications.nodes) || [];
  const applications = rawApplications.map(app => ({
      type: app.__typename || null,
      allocationMethod: app.allocationMethod || null,
      targetType: app.targetType || null,
      amount: fromMoneyV2(app.totalAllocatedAmount) || 0
    }));
  // Prefer Shopify's own allocation totals (correct for automatic, code and
  // custom discounts, in any currency and market); fall back to the
  // subtotal-difference arithmetic when they are not returned.
  const allocated = applications
    .filter(a => a.targetType !== 'SHIPPING_LINE')
    .reduce((s, a) => s + a.amount, 0);
  const discountAmount = applications.length
    ? allocated
    : Math.max(0, subtotal - (costSubtotal !== null ? costSubtotal : subtotal));
  const noteParts = [];
