 ...
 * Catalog data comes from Shopify (ctx.chrome); filtering, sorting, facet
 * counts, chips, load-more and pagination are the storefront's own
 * presentation logic, unchanged.
 */
const settings = require('../settings');
const catalog = require('../shopify/catalog');
const ui = require('../ui');
const { esc, attr, icon, productGrid, breadcrumbs, pagination, sectionHeader } = ui;
...
/** 'all' and 'sale' are virtual collections built from the live catalog. */
function collectionProducts(ctx, handle) {
  const products = ctx.chrome.products;
  if (!handle || handle === 'all') return products;
  const collection = ctx.chrome.collections.find(c => c.handle === handle);
  if (collection) {
...
function applyFilters(products, q) {
  ...
  switch (q.sort) {
    ...
    case 'best-selling': out.sort((a, b) => b.rating.count - a.rating.count); break;
    default: break;
  }
  return out;
}
...
function facetCounts(products) {
  ...
  for (const p of products) {
    ...
    types.set(p.type, (types.get(p) || 0) + 1);
    p.tags.filter(t => !t.startsWith('badge:')).forEach(t => tags.set(t, (tags.get(t) || 0) + 1));
  }
...
function filterSidebar(facets, q, base, totalProducts) {
  const keep = (name, value) => `<input type="hidden" name="${attr(name)}" value="${attr(value)}">`;
  return `<form class="filters" data-filters action="${attr(base)}" method="get" aria-labelledby="filters-title">
    <h2 class="sr-only" id="filters-title">Filter and sort</h2>
    ${q.sort ? keep('sort', q.sort) : ''}
    ${q.q ? keep('q', q.q) : ''}
    <div class="filters__group">
      <h3>Availability</h3>
      <label class="check"><input type="radio" name="availability" value="" ${!q.availability ? 'checked' : ''}><span>All products (${totalProducts})</span></label>
    ...
    <details class="filters__group" open>
      ...
      <div class="swatch-filters">
        ${facets.colors.map(([name, count]) => `<label class="swatch-filter">
          ...
          <span class="swatch" style="--sw:${attr(settings.swatchHex(name))}"></span>
...
function renderCollection(ctx, handle) {
  const collection = handle === 'all' ? null : ctx.chrome.collections.find(c => c.handle === handle);
  if (handle !== 'all' && !collection && handle !== 'sale') return null;
  const base = handle === 'all' ? '/collections/all' : `/collections/${handle}`;
  const all = collectionProducts(ctx, handle);
  ...
  return {
    ...
    <div class="collection-hero">
      ${image ? `<img src="${attr(image)}" alt="" width="1600" height="600" loading="lazy">` : ''}
    ...
    <div class="wrap collection">
      <aside class="collection__aside ${sidebarOpen ? 'is-open' : ''}" data-filter-panel>
        ${filterSidebar(facets, ctx.query, base, ctx.chrome.products.length)}
      </aside>
...
async function renderSearch(ctx) {
  ...
  let products = [];
  if (q) {
    // Shopify search is the source of truth; local filters still apply on top
    products = await catalog.searchProducts(q, 48).catch(() => []);
    if (!products.length) products = applyFilters(ctx.chrome.products, { q });
    else products = applyFilters(products, ctx.query);
  }
  const collections = q ? ctx.chrome.collections.filter(c => `${c.title} ${c.description}`.toLowerCase().includes(q.toLowerCase())) : [];
  const [pages, articles] = q ? await Promise.all([
    catalog.getPages().catch(() => []),
    catalog.getArticles().catch(() => [])
  ]) : [[], []];
  const pageHits = q ? pages.filter(p => p.title.toLowerCase().includes(q.toLowerCase())).slice(0, 5) : [];
  const postHits = q ? articles.filter(p => `${p.title} ${p.excerpt}`.toLowerCase().includes(q.toLowerCase())).slice(0, 4) : [];
  const total = products.length + collections.length + postHits.length + pageHits.length;
  const popular = ctx.chrome.products.slice(0, 4);
  return {
    ...
    <div class="wrap search-results">
      ...
      : total === 0 ? `<div class="empty-state">
        ...
        ${postHits.length ? `<section class="sec"><h2 class="sec-title">Journal</h2><ul class="result-list">${postHits.map(p => `<li><a href="/blogs/journal/${attr(p.handle)}"><strong>${esc(p.title)}</strong><span>${esc(p.excerpt)}</span></a></li>`).join('')}</ul></section>` : ''}
        ${pageHits.length ? `<section class="sec"><h2 class="sec-title">Help &amp; information</h2><ul class="result-list">${pageHits.map(p => `<li><a href="/pages/${attr(p.handle)}"><strong>${esc(p.title)}</strong><span>Support and policy information</span></a></li>`).join('')}</ul></section>` : ''}`}
    </div>`
  };
}
