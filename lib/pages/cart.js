...
function lineRow(line) {
  const product = store.all('products').find(p => p.variants.some(v => v.id === line.variantId));
  const url = product ? `/products/${product.handle}` : '#';
    ...
    <div class="cart-page__info">
      <div class="cart-page__info-top">
        <div>
          ...
          <p class="muted">${esc(line.color)} · ${esc(line.size)} · ${esc(line.sku)}</p>
          ${line.personalization && line.personalization.text ? `<p class="cart-line__monogram">${icon('spark', { size: 13 })} ${esc(line.personalization.label || 'Monogram')}: <strong>${esc(line.personalization.text)}</strong>${line.personalization.placement ? ` · ${esc(line.personalization.placement)}` : ''} — <em>${esc(line.personalization.note || 'personalised, not returnable')}</em></p>` : ''}
      ...
      <div class="cart-page__actions">
        ...
        <button class="link-inline" type="button" data-wish="${attr(product ? product.handle : '')}">${icon('heart', { size: 15 })} Save for later</button>
  ...
  return `<aside class="order-summary" data-summary>
    ...
    <form class="discount-form" data-discount-form action="/api/cart/discount" method="post">
      ...
      <div class="discount-form__row">
        <input id="discount" type="text" name="code" placeholder="WELCOME10" value="${attr(cart.discount ? cart.discount.code : '')}" ${cart.discount ? 'readonly' : ''} autocomplete="off">
      ...
      <p class="discount-form__hint">Try <button type="button" class="link-inline" data-fill-code="WELCOME10">WELCOME10</button>, <button type="button" class="link-inline" data-fill-code="FREESHIP">FREESHIP</button> or <button type="button" class="link-inline" data-fill-code="CAPSULE20">CAPSULE20</button>.</p>
    </form>
    <dl class="totals">
      ...
      <div><dt>Shipping <small>${esc(cart.shipping.label)}${cart.freeShipping.qualified ? ' · free' : ''}</small></dt><dd data-sum-shipping>${cart.shipping.amount === 0 ? 'Free' : money(cart.shipping.amount)}</dd></div>
      <div><dt>${esc(cart.tax.name)}</dt><dd data-sum-tax>${money(cart.tax.amount)}</dd></div>
    ...
    <ul class="summary-notes">
      ...
      <li>${icon('shield', { size: 15 })} Sandbox payments — no real cards are charged</li>
    </ul>
  </aside>`;
}
...
function render(ctx) {
  const { cart } = ctx;
  if (!cart.count) {
    const picks = store.all('products').slice(0, 3);
    return {
  ...
  const related = store.all('products').filter(p => !cart.lines.some(l => l.productId === p.id)).slice(0, 3);
  return {
    title: 'Your cart',
