'use strict';
/**
 * Homepage — editorial VENNIX landing experience.
 * Data: Shopify catalog via lib/shopify/catalog (through ctx.chrome /
 * ctx.articles). The section composition and markup are unchanged.
 */
const ui = require('../ui');
  ...
  return `<section class="hero" aria-label="Capsule 01">
    ...
    <div class="wrap hero__inner">
      ...
      <ul class="hero__trust">
        ...
        <li>${icon('lock', { size: 16 })} Secure Shopify checkout</li>
      </ul>
    </div>
...
function categoryTriptych(collections = []) {
  const count = h => {
    const c = collections.find(x => x.handle === h);
    return c ? c.productHandles.length : 0;
  };
  const picks = [
    { handle: 'women', title: "Women's", copy: 'Compression, bras and layers', image: '/images/lookbook-2.jpg', count: count('women') },
    { handle: 'men', title: "Men's", copy: 'Heavyweight fleece and technical layers', image: '/images/p-atlas-hoodie.jpg', count: count('men') },
    { handle: 'active', title: 'Active', copy: 'Tested at pace by 30 wearers', image: '/images/lookbook-1.jpg', count: count('active') }
  ];
...
/**
 * The honest numbers band. Counts only what Shopify actually reports: styles
 * and options from the live catalog, reviews only where the store (reviews
 * app / metafields) provides them.
 */
function storePulse(products) {
  const styles = products.filter(p => !p.digital);
  const variants = styles.reduce((sum, p) => sum + p.variants.length, 0);
  const allReviews = products.flatMap(p => p.reviews || []);
  const avg = allReviews.length ? allReviews.reduce((s, r) => s + r.rating, 0) / allReviews.length : 0;

  const events = [];
  const newest = allReviews.slice().sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0))[0];
  if (newest) events.push(`${newest.author} reviewed a piece — ${newest.rating}★`);
  const unrated = styles.filter(p => p.rating.count === 0);
  ...
  events.push(`Free repairs for two years on all ${styles.length} apparel styles`);

  return `<section class="pulse" aria-label="Store facts">
    <div class="wrap pulse__inner">
      <div>
        <p class="pulse__label">The honest numbers</p>
        <h2 class="pulse__heading">${allReviews.length} reviews. No invented ones.</h2>
      ...
      <div class="pulse__stats">
        <div class="pulse__stat">
          <strong data-count-to="${styles.length}">0</strong>
          <span>Styles in capsule 01</span>
        </div>
        <div class="pulse__stat pulse__stat--reviews">
          <strong data-count-to="${allReviews.length}">0</strong>
...
function reviewsSection(products) {
  const allReviews = products.flatMap(p => (p.reviews || []).map(r => ({ ...r, productHandle: p.handle, productTitle: p.title })));
  const reviews = allReviews.slice().sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0)).slice(0, 6);
  const avg = allReviews.length ? allReviews.reduce((s, r) => s + r.rating, 0) / allReviews.length : 0;
  const countLabel = allReviews.length
    ? `Rated ${avg.toFixed(1)} out of 5 from ${allReviews.length} ${allReviews.length === 1 ? 'review' : 'reviews'}`
    : 'Be the first to review';
  return `<section class="sec sec--sand" aria-labelledby="h-reviews">
    <div class="wrap">
      ${sectionHeader({ eyebrow: 'Customer reviews', title: countLabel, copy: allReviews.length ? `Every review is written by a real customer — ${allReviews.length} so far, and we publish the critical ones too.` : 'Reviews appear here once real customers share them.', align: 'center' })}
      <div class="review-grid">
        ${reviews.length ? reviews.map(r => `<figure class="review">
        ...
        </figure>`).join('') : `<p class="muted" style="grid-column:1/-1;text-align:center">No reviews yet — this store publishes only reviews written by real customers.</p>`}
      </div>
    </div>
  </section>`;
}

function journalTeasers(articles = []) {
  ...
  return `<section class="sec wrap" aria-labelledby="h-journal">
    ...
    <div class="post-grid">
      ${articles.slice(0, 3).map(p => `<article class="post-card">
...
async function render(ctx) {
  ...
  const inCollection = handle => {
    const c = collections.find(x => x.handle === handle);
    if (!c) return [];
    const byHandle = new Map(products.map(p => [p.handle, p]));
    return c.productHandles.map(h => byHandle.get(h)).filter(Boolean);
  };
  const bestsellers = inCollection('bestsellers');
  const newIn = inCollection('new-in');
  const spotlightProduct = products.find(p => p.handle === 'atlas-heavyweight-hoodie') || products[0];
  const articles = await catalog.getArticles().catch(() => []);
  const jsonLd = [
    {
  ...
  return {
    ...
    content: `${hero()}
      ...
      ${spotlightProduct ? spotlight(spotlightProduct) : ''}
      ...
      ${storePulse(products)}
      ${reviewsSection(products)}
      ${journalTeasers(articles)}
      ${lookbookStrip()}
      ${newsletterBand()}`
  };
}
