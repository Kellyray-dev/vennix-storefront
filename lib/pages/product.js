...
const { esc, attr, icon, stars, productGrid, breadcrumbs, money, sectionHeader } = ui;
...
/**
 * Optional paid personalisation, sold the way a knitwear house sells a
 * monogram: off by default, priced per unit, previewed live, and honest about
 * the fact that personalised pieces cannot go back on the shelf.
 */
function personalizationBlock(product) {
  const config = personalize.configFor(product);
  if (!config) return '';
  const slider = config.maxChars;
  return `<fieldset class="picker monogram" data-monogram data-price="${attr(config.price)}" data-label="${attr(config.label)}" data-max="${attr(slider)}">
    <legend class="monogram__legend">
      <label class="check monogram__toggle">
        ...
        <span>${esc(config.copy.heading)} <em>+${money(config.price)}</em></span>
      </label>
    </legend>
...
/** The "what's my size?" entry point, rendered under the size picker. */
function fitTrigger() {
...
/**
 * Shop the look. Complements are chosen from what actually pairs with the piece,
 * with every in-stock item addable in one request — and any bundle saving comes
 * from a real, evaluated discount code rather than a made-up badge.
 */
function styleRail(product) {
  const look = style.lookFor(product, 3);
    ...
    <div class="stylerail__grid">
        ...
        ${items.map((p, i) => {
          ...
          return `<li class="stylerail__item ${i === 0 ? 'is-base' : ''}">
            <a class="stylerail__media" href="/products/${attr(p.handle)}">
              <img src="${attr(p.images[0].src)}" alt="${attr(p.images[0].alt || p.title)}" width="300" height="360" loading="lazy">
            ...
            <div class="stylerail__meta">
              ...
              <span class="stylerail__avail">${first.stock > 0 ? 'In stock' : 'Sold out'}</span>
            </div>
          </li>`;
        }).join('')}
      </ol>
      <aside class="stylerail__summary">
        ...
        <button class="btn btn--primary btn--block" type="button"
                data-add-look="${attr(JSON.stringify(items.map(p => ({ handle: p.handle, variantId: (p.variants.find(v => v.stock > 0) || p.variants[0]).id }))))}">
...
function accordions(product) {
  const items = [
    ...
    { title: 'Features & materials', body: `<ul class="feature-list">${product.features.map(f => `<li>${icon('check', { size: 15 })} ${esc(f)}</li>`).join('')}</ul><dl class="spec-list"><dt>Materials</dt><dd>${esc(product.materials)}</dd><dt>Fit</dt><dd>${esc(product.fit)}</dd><dt>Weight</dt><dd>${product.shippingWeight} g</dd><dt>Product code</dt><dd>${esc(product.variants[0].sku)}</dd></dl>` },
    { title: 'Care instructions', body: `<p>${esc(product.care)}</p><p class="muted">Caring for technical fabrics: skip fabric softener (it blocks the wicking channels), wash with like colours and hang dry where you can. Repairs are free for two years — <a href="/pages/contact?topic=Repairs">book one here</a>.</p>` },
    { title: 'Shipping & returns', body: `<ul class="feature-list"><li>${icon('truck', { size: 15 })} Free standard shipping over ${ui.freeShipText()} (4–6 business days)</li><li>${icon('refresh', { size: 15 })} 30-day free returns and free exchanges</li><li>${icon('lock', { size: 15 })} Secure checkout — PCI-DSS, tokenised cards</li><li>${icon('box', { size: 15 })} Studio pickup available in Brooklyn within 2 hours</li></ul><p class="muted">Full details in <a href="/pages/shipping-returns">Shipping &amp; returns</a>.</p>` }
  ];
...
function reviewsBlock(product) {
  const all = store.all('reviews').filter(r => r.productHandle === product.handle && r.status === 'published');
  ...
  return `<section class="sec reviews-block" id="reviews" aria-labelledby="h-reviews-product">
    <div class="wrap">
      ...
      <ul class="review-list">
        ${all.length ? all.slice().sort((a, b) => b.helpful - a.helpful).map(r => `<li class="review-item">
          <div class="review-item__head">
            <div>
              ${stars(r.rating, { showValue: false })}
              <strong>${esc(r.title)}</strong>
            </div>
            <span class="review-item__date">${new Date(r.createdAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</span>
          </div>
          <p>${esc(r.body)}</p>
          <div class="review-item__foot">
            ...
            <button class="link-inline" type="button" data-helpful="${attr(r.id)}">${icon('heart', { size: 14 })} Helpful (${r.helpful})</button>
          </div>
      ...
      <details class="review-form-wrap">
        ...
        <form class="review-form" data-review-form data-product="${attr(product.handle)}">
          ...
          <p class="form-msg" data-form-msg role="status"></p>
          <p class="muted">Reviews are moderated and published within one business day.</p>
        </form>
      </details>
    </div>
  </section>`;
...
function stickyBar(product) {
  ...
  return `<div class="sticky-atc" data-sticky-atc hidden>
    <div class="wrap sticky-atc__inner">
      <img src="${attr(product.images[0].src)}" alt="" width="64" height="64" loading="lazy">
...
function render(ctx, handle) {
  const product = store.find('products', p => p.handle === handle && p.status === 'active');
  if (!product) return null;

  // the trail follows the product's own collection — category is a display hint, not a route
  const primaryCollection = (product.collections || [])
    .map(id => ctx.db.collections.find(c => c.id === id || c.handle === id))
    .find(Boolean);
  ...
  const jsonLd = [
    {
      ...
      image: product.images.map(i => `https://${ctx.settings.domain}${i.src}`),
      sku: product.variants[0].sku, brand: { '@type': 'Brand', name: product.vendor },
      ...
      review: store.all('reviews').filter(r => r.productHandle === product.handle && r.status === 'published').slice(0, 5).map(r => ({
        '@type': 'Review', reviewRating: { '@type': 'Rating', ratingValue: r.rating }, author: { '@type': 'Person', name: r.author }, name: r.title, reviewBody: r.body, datePublished: r.createdAt.slice(0, 10)
      }))
    },
  ...
  const wanted = store.all('products').filter(p => p.collections.includes('bestsellers') && p.handle !== product.handle).slice(0, 4);
  return {
    title: product.title,
